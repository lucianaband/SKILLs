...
> **Adaptado para LLM**: Crie arquivos localmente e use `present_files` para disponibilizar ao usuário.

Este fluxo de trabalho orienta você na criação de um épico usando `templates/product/prod-epic-template.md`. épicos agrupam histórias de usuários relacionadas que oferecem uma funcionalidade significativa.
...
## Resultado final
- Por padrão, use o modelo `templates/product/prod-epic-template.md` integralmente
- Siga a estrutura do modelo exatamente como definida, não pule passos, a não ser que o usuário solicite explicitamente
- Nomeie o arquivo usando a convenção: `output/{epic-ID}-{epic-name}.md` (se não existir uma PRD para pegar o nome, ignore esse prefixo)
